...
def createTraversalPath(starting_room):
    traversalPath = []
    # create a stack
    stack = Stack()
    # create hansel stack for return directions
    hansel_stack = Stack()
    # push starting room onto stack
    stack.push(starting_room)
    # set player current room to world starting room
    player.currentRoom = starting_room
    ...
    while len(visited.keys()) < len(roomGraph.keys()) :
        # pop room off stack and set to current room
        current_room = stack.pop()
        # if current room has some directions that haven't been explored
        rooms.append(current_room.id)
        if current_room.id not in visited.keys() or '?' in visited[current_room.id].values():
            # call getExists on room
            exits = current_room.getExits()


            # set visited dictionary key to current room id and values to dictionary of exists each with question mark
            if current_room.id not in visited.keys():
                visited[current_room.id] = { i : '?' for i in exits }
            
            if previous_room != None and direction_from != None:
                visited[current_room.id][direction_from] = previous_room
                previous_room = None
                direction_from = None

            # loop through values and set direction to first value with question mark
            for (key, value) in visited[current_room.id].items():
                if value == '?':
                    # move player in this direction
                    traversalPath.append(key)
                    player.travel(key)
                    # add opposite direction to hansel_stack 
                    hansel_stack.push(opposite[key])
                    # set previous rooms direction to the new rooms id
                    visited[current_room.id][key] = player.currentRoom.id
                    # set opposite direction as id of previous room
                    direction_from = opposite[key]
            ...
            # keep popping items of hansel stack and moving player until room is found with some ? marks
            direction = hansel_stack.pop()
...
traversalPath = createTraversalPath(world.startingRoom)
# TRAVERSAL TEST
visited_rooms = set()
